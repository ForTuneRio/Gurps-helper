...
<div>
    ...
    <div v-if="result" class="p-4 bg-gray-50 rounded-md">
        ...
        <div class="grid grid-cols-2 gap-4">
            <p><strong>Spell:</strong> {{ result.spellName }}</p>
            <p><strong>Required Energy:</strong> {{ result.requiredEnergy }}</p>
            <p><strong>Effective Cap:</strong> {{ result.cap }}</p>
            <p><strong>Path Skill:</strong> {{ result.skill }}</p>
        </div>

        <div>
            <p><strong>Total Energy Gathered:</strong> <span class="text-xl font-bold">{{ result.totalEnergy }}</span></p>
            <p><strong>Success:</strong>
            <span :class="result.success ? 'text-green-600 font-bold' : 'text-red-600 font-bold'">
                {{ result.success ? 'YES' : 'NO' }}
            </span>
            </p>
            <p v-if="result.criticalFailure" class="text-red-600 font-bold">
            <strong>CRITICAL FAILURE!</strong> Botch occurred!
            </p>
        </div>

        <div>
            <h5 class="font-semibold mb-2">Casting Rolls ({{ result.rolls.length }} attempts):</h5>
